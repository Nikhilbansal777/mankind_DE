...
DEFAULT_TABLES = [
    ...
    "payments", "address", "category",
    "cart_item", "wishlist",
    # NOTE: "order_payments" removed (table dropped)
    # add later if needed: "address", "category"
]
...
def main():
    ...
    for t in tables:
        try:
            ...
            summary[t] = {
                ...
                "audit_path": audit.get("audit_path"),   # will be present now after changing in run_cleaning_common.py
            }
            print(f"[CLEANING][{t}] ✅ {summary[t]['rows']} rows -> {summary[t]['output_path']}")
            print(f"[CLEANING][{t}] 🧾 audit -> {summary[t]['audit_path']}")
        except ModuleNotFoundError as e:
            print(f"[CLEANING][{t}] SKIP (no cleaner): {e}")
            skipped.append(t)
    ...
    if summary:
        ...
        for t, info in summary.items():
            ...
            if info.get("audit_path"):
                print(f"      audit: {info['audit_path']}")
    if skipped:
