# MKM_Glue_tranformations/src/common/semantic_utils.py

import json, os
from pathlib import Path
from typing import Iterable, Set, Dict, Sequence
from pyspark.sql import DataFrame
from src.utils.path_utils import get_lineage_output_path
...
def _lineage_columns(table: str, lineage_stage: str = "semantic") -> Set[str]:
    """
    Load set of columns for a table from lineage JSON if present; fallback to empty set.
    We expect a small file at:
      outputs/lineage/semantic/schemas/<table>_lineage.json
    with shape like: {"columns": ["col1", "col2", ...]} or {"schema": [...]}.
    """

    try:
        # keep area-specific separation via lineage_stage
        path = get_lineage_output_path(lineage_stage, "schemas", f"{table}_lineage.json")
...
def _suggest_renames(
    ...
    for c in overlap:
        tgt = f"{right_table}__event_ts" if c == "__event_ts" else f"{right_table}_{c}"
        rename_map[c] = tgt
    return rename_map
...
def _write_rename_audit(left_table: str, right_table: str, rename_map: Dict[str, str], lineage_stage: str = "semantic") -> None:
    """Persist what we renamed for traceability under lineage/join_renames/"""
    if not rename_map:
        return
    folder = Path(get_lineage_output_path(lineage_stage, "join_renames"))
...
def safe_join_with_lineage(
    ...
    lcols = _lineage_columns(left_table, lineage_stage) | set(left.columns)
    rcols = _lineage_columns(right_table, lineage_stage) | set(right.columns)
    ...
    _write_rename_audit(left_table, right_table, rename_map, lineage_stage=lineage_stage)
    return left.join(right, on_expr, how)

def write_schema_lineage(
    table: str,
    cols: Sequence[str],
    lineage_stage: str = "semantic"
) -> str:
    """
    Save a simple schema snapshot JSON for collision planning & audit.
    Writes to:
      outputs/lineage/<lineage_stage>/schemas/<table}_lineage.json
    Returns the file path.
    """
    folder = Path(get_lineage_output_path(lineage_stage, "schemas"))
    folder.mkdir(parents=True, exist_ok=True)
    out = folder / f"{table}_lineage.json"
    with open(out, "w", encoding="utf-8") as f:
        json.dump({"columns": list(cols)}, f, indent=2)
    return str(out)
