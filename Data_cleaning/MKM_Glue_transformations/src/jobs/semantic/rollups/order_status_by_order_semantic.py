# MKM_Glue_tranformations/src/jobs/semantic/rollups/order_status_by_order_semantic.py
...
def main():
    ...
    try:
        ...
        # ------------------------------------------------------------------
        # Tolerant status column selection (fix for missing 'to_status')
        # ------------------------------------------------------------------
        status_col = "to_status" if "to_status" in hist.columns else "status"

        # Window to pick latest transition per order (created_at DESC)
        ...
        metrics = {
            ...
            "inputs": {
                "order_status_history": {"rows": hist.count()}
            },
